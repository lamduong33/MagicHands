...
class ConvNet:
    ...
    def build(self):
        ...
        self.model.add(keras.layers.Conv2D(64, kernel_size=[
                       3, 3], padding='same', activation='relu'))
        self.model.add(keras.layers.Conv2D(128, kernel_size=[
                       3, 3], padding='same', activation='relu'))
        self.model.add(keras.layers.MaxPool2D(pool_size=[3, 3]))

        # Two convolutional layers followed by a pooling layer
        self.model.add(keras.layers.Conv2D(256, kernel_size=[
                       3, 3], padding='same', activation='relu'))
        self.model.add(keras.layers.Conv2D(512, kernel_size=[
                       3, 3], padding='same', activation='relu'))
        self.model.add(keras.layers.MaxPool2D(pool_size=[3, 3]))
    ...
    def predict_generator(self):
        ...
        results = pandas.DataFrame(
            {"Filename:": file_names, "Predictions": final_predictions})
